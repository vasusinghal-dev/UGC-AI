...
const ProjectCard = ({
  ...
  const { handleDelete, togglePublish } = useProjectCard(setGenerations);
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition group">
        ...
        >
          ...
          <div className="absolute left-3 top-3 flex gap-2 items-center">
            {gen.isGenerating && (
              <span className="text-sm px-2 py-1 bg-yellow-600/30 rounded-full">
                Generating
              </span>
            )}
            {gen.isPublished && (
              <span className="text-sm px-2 py-1 bg-green-600/30 rounded-full">
                Published
              </span>
            )}
          </div>
